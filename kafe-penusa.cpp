        ...
        if (hdcPrinter) {
            ...
            if (StartDoc(hdcPrinter, &di) > 0) {
                if (StartPage(hdcPrinter) > 0) {
                    ...
                    int columnSpacing = 100; // Spasi antar kolom
                    int totalSpacing = 0;
                    ...
                    TextOut(hdcPrinter, x, y, "Nama Barang", 11);
                    TextOut(hdcPrinter, x + columnSpacing, y, "Jumlah", 6);
                    TextOut(hdcPrinter, x + 2 * columnSpacing, y, "Harga Satuan", 12);
                    TextOut(hdcPrinter, x + 3 * columnSpacing, y, "Total Harga", 11);
                    y += 25;
                    ...
                    for (const auto& data : penjualanData) {
                        ...
                        if (pos != std::string::npos) {
                            std::string namaPesanan = data.substr(pos + 6);
                            std::string hargaSatuan = data.substr(data.find("Harga Satuan: ") + 14, data.find("Jumlah: ") - (data.find("Harga Satuan: ") + 14));
                            std::string jumlah = data.substr(data.find("Jumlah: ") + 8, data.find("Total Harga: ") - (data.find("Jumlah: ") + 8));
                            std::string totalHarga = data.substr(data.find("Total Harga: ") + 13);

                            // Menampilkan baris item penjualan
                            TextOut(hdcPrinter, x, y, namaPesanan.c_str(), namaPesanan.length());
                            TextOut(hdcPrinter, x + columnSpacing, y, jumlah.c_str(), jumlah.length());
                            TextOut(hdcPrinter, x + 2 * columnSpacing, y, hargaSatuan.c_str(), hargaSatuan.length());
                            TextOut(hdcPrinter, x + 3 * columnSpacing, y, totalHarga.c_str(), totalHarga.length());
                            y += 25; // Baris berikutnya
                        }
                    }

                    // Garis Horizontal sebelum total keseluruhan
                    MoveToEx(hdcPrinter, x, y, NULL);
                    LineTo(hdcPrinter, x + 500, y); // Membuat garis horizontal
                    y += 5;
                    ...
                    // Garis Horizontal setelah Total
                    MoveToEx(hdcPrinter, x, y, NULL);
                    LineTo(hdcPrinter, x + 500, y); // Membuat garis horizontal
                    y += 15;
